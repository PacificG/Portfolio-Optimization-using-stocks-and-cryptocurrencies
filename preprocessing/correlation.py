...
def correlation(csv_path, num_stocks, window_length):
    df = pd.read_csv(csv_path)
    print(df.head())
    corr_table = df[['date', 'close', ' Name']].set_index('date', inplace=True).\
            pivot_table(index='date', columns=' Name', values='close')
    corr_table = corr_table.rolling(window_length).corr()
    corr_table['stock1'] = corr_table.index
    corr_table = corr_table.melt(id_vars = 'stock1', var_name = "stock2").reset_index(drop = True)[corr_table['stock1'] < corr_table['stock2']].dropna()
...
if __name__ == '__main__':
    ...
    print(correlation(args.input, args.Num_Stocks, args.window_length))
